...
// Calcolo del costo totale dei prezzi
for (let i = 0; i < prices.length; i++) {
  totalCost += prices[i];
}

// Applicazione dello sconto se l'utente è un ambassador
if (utenteCheEffettuaLAcquisto.isAmbassador) {
  const discount = totalCost * 0.3;
totalCost -= discount;
}

// Aggiunta del costo di spedizione se necessario
if (totalCost <= 100) {
...
const users = []; // Array vuoto per gli utenti
users.push(marco, paul, amy); // Aggiungi gli utenti all'array
